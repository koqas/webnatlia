...
class UsersController extends AppController
{
	...
	function beforeFilter()
	{
		...
		$supervisors = $this->{$this->ModelName}->Supervisor->find('list', array(
			'conditions' => array(
				'Supervisor.status' => 1
			)
		));
	...
	function ListItem()
	{
		...
		if(!empty($this->request->data))
		{
			...
			if(!empty($this->request->data['Search']['supervisor_id']))
			{
				$cond_search["{$this->ModelName}.supervisor_id"]					=	$this->data['Search']['supervisor_id'];
			}
	...
	function Edit($ID=NULL)
	{
		...
		else
		{
			...
			if($postData[$this->ModelName]['new_password'] != "") {
				$postData[$this->ModelName]['password'] = $postData[$this->ModelName]['new_password'];
			}
			
			$this->{$this->ModelName}->set($postData);
	...
	public function getListUserBySupervisorId($id = null) {
	
    	$data = $this->{$this->ModelName}->find('list', array(
    		'conditions' => array(
    			'Supervisor.id' => $id,
    		),
    		'recursive' => 0
