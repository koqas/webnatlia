...
$(document).ready(function(){
	...
	$("#SearchRegionId").change(changeRegion);
...
function changeRegion() {
	var id = $('#SearchRegionId').val();
	$("#loaderRegionId").html('loading');
	removeDropZoneValue();
	$.getJSON("<?php echo $this->webroot ?>Zones/getListZoneByRegionId/"+id+".json", function(data){
		if(data.data.Error) {
			
		} else {
			$.each(data.data, function (index, value) {  
				var listItem = $("<option></option>").val(index).html(value);  
				$("#SearchZoneId").append(listItem);  
			});
		}
